Shader "SeganX/Toon/VertexLit"
{
	Properties
	{
		...
		[Space(20)]
		[Toggle] Feature_Rim("Rim", Float) = 0
	...
	SubShader
	{
		...
		Pass
		{
			...
			vertex_output vert(vertex_input v)
			{
				...
				float dot_light = dot(_WorldSpaceLightPos0, o.normal);
				float3 ambient = ShadeSH9(float4(o.normal, 1.0f)) * _Ambient;
...
#if FEATURE_SPECULAR_ON
				float3 half_view = normalize(_WorldSpaceLightPos0 + o.view_dir);
				float NdotH = dot(o.normal, half_view);
				o.specular = pow(NdotH, _Glossiness);
#endif
			...
			fixed4 frag(vertex_output v) : SV_Target
			{
				...
				float light = smoothstep(0, 0.05f, v.shade.a * shadow);
				float3 attenu = _LightColor0.rgb * light;

#if FEATURE_SPECULAR_ON
				float3 specular = attenu * smoothstep(0.005, 0.01, v.specular) * _SpecularColor;
#endif

#if FEATURE_RIM_ON
				float3 normal = normalize(v.normal);
				float3 view_dir = normalize(v.view_dir);
