...
Shader "SeganX/UI/Surface_Move"
{
    Properties
    {
        _MainColor("Sprite Color", Color) = (1.0,1.0,1.0,1.0)
        [HideInInspector]_MainTex("Sprite (RGB)", 2D) = "white" {}
        _SurfColor("Surface Color", Color) = (1.0,1.0,1.0,1.0)
        ...
        SubShader
        {
            ...
            Pass
            {
                ...
                {
                    fixed4 c = tex2D(_MainTex, i.uv0) * i.col * _MainColor;

                    fixed4 s = tex2D(_SurfTex, i.uv1) * _SurfColor;
