    ...
    public class PluginEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            else
            {
                ...
                EditorGUILayout.HelpBox("Source folder is not valid!\nPlease set source folder in order to setup plugin.", UnityEditor.MessageType.Error);
            ...
            EditorGUILayout.Space();
            EditorGUILayout.LabelField("Product Name: " + plugin.productName);
        ...
        private static void OnActivate(Plugin obj)
        {
            ...
            foreach (var item in obj.files)
                try
                {
                    string src = (obj.folder + item).PreparePath(), dest = (Application.dataPath + item).PreparePath();
        ...
        private static void OnDeactivate(Plugin obj)
        {
            ...
            foreach (var item in obj.files)
                try
                {
                    string path = (Application.dataPath + item).PreparePath();
        ...
        public void OnUpdateSource(Plugin obj)
        {
            foreach (var item in obj.files)
                try
                {
                    string src = (Application.dataPath + item).PreparePath(), dest = (obj.folder + item).PreparePath();
                    if (Directory.Exists(src))
                    {
                        if (!Directory.Exists(dest))
                            Directory.CreateDirectory(dest);
        ...
        {
#if UNITY_ANDROID
            var buildTarget = UnityEditor.BuildTargetGroup.Android;
#elif UNITY_IOS
            var buildTarget = UnityEditor.BuildTargetGroup.iOS;
#elif UNITY_STANDALONE
            var buildTarget = UnityEditor.BuildTargetGroup.Standalone;
#elif UNITY_WEBGL
            var buildTarget = UnityEditor.BuildTargetGroup.WebGL;
#endif
...
#elif UNITY_IOS
            var targetGroup = UnityEditor.BuildTargetGroup.iOS;
#elif UNITY_STANDALONE
            var targetGroup = UnityEditor.BuildTargetGroup.Standalone;
#elif UNITY_WEBGL
            var targetGroup = UnityEditor.BuildTargetGroup.WebGL;
#endif
